...
class Start_windows:
    def __init__(self, window, window_title, image_path=r"logo\logo15.jpg"):
        self.window = window
        self.window.title(window_title)
        # self.window.iconbitmap('logo.ico')
        self.window.iconphoto(False, tkinter.PhotoImage(file=r'logo\logo.png'))
        self.is_closed = True
...
class Window(threading.Thread):
    ...
    def run(self):
        ...
        # Load an image using OpenCV
        self.cv_img = cv2.cvtColor(cv2.imread(r"logo\logo15.jpg"), cv2.COLOR_BGR2RGB)
        # self.cv_img = cv2.Canny(, 50, 100)640w, 480h
...
# App(tkinter.Tk(), "Appright")
app = Start_windows(tkinter.Tk(), "Appright")
